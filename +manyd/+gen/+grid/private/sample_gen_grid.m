function [x_samp] = sample_gen_grid(nx_samp,n_dim)
%*****************************************************************************80
%
%% SAMPLE_GEN_GRID generates a regular grid of points in any dimension
%
%  Parameters:
%
...
if n_dim == 1
    dx = 1/nx_samp;
    x_samp = linspace(dx/2, 1-dx/2, nx_samp)';
...
if (B == 1)
    for i=1:M
        output(1 + (i - 1)*N:i*N, 1) = (i - 1)*dx + dx/2;
    end
else
    for i=1:M
        output(1 + (i - 1)*N:i*N, 1) = (i - 1)*dx + dx/2;
